...
const Product = ({ item, provider, account, chainazon, togglePop }) => {
  const [order, setOrder] = useState(null);
  const [hasBought, setHasBought] = useState(false);

  const fetchDetails = async () => {
    const events = await chainazon.queryFilter("Buy");
    ...
    const order = await chainazon.orders(account, orders[0].args.orderId);
    setOrder(order);
  };
    ...
    <div className="product">
      <div className="product__details">
        ...
        <div className="product__order">
          ...
          <p>
            <small>Ships from</small> Chainazon
          </p>
          <p>
            <small>Sold by</small> Chainazon
          </p>
